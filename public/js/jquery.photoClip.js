...
(function ($) {
    ...
    $.fn.photoClip = function (option) {
        if (!window.FileReader) {
            $('#my-message').text("您的浏览器不支持图片裁剪,请更换浏览器!");
    ...
    function photoClip(container, option) {
        ...
        $file.change(function () {
            ...
            if (!/image\/\w+/.test(this.files[0].type)) {
                ...
                $('#my-message').text("图片格式不正确,请选择正确格式的图片文件!");
                $('#textModal').modal('show');
                return false;
            } else {
                ...
                fileReader.onerror = function (e) {
                    //alert("图片加载失败");
                    $('#my-message').text("图片加载失败,请返回重新选择!");
                    $('#textModal').modal('show');
        ...
        function init() {
            ...
            var $mask_left = $("<div class='photo-clip-mask-left'>").css({
                ...
                "background-color": "rgb(144, 202, 190)"
            }).appendTo($mask);
